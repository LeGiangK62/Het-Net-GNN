...
class Ue2Ap(MessagePassing):
    def __init__(self, node_dim: Dict, edge_dim, out_node_dim, metadata: Metadata, aggr='mean', **kwargs):
        ...
        for edge_type in metadata[1]:
            src_type, _, dst_type = edge_type
            self.lin_edge_compact['__'.join(edge_type)] = mlp([edge_dim - 1, 16, out_dim_src[src_type]])
...
class PowerConv_wAP(MessagePassing):
    def __init__(self, node_dim: Dict, edge_dim, out_node_dim, metadata: Metadata, aggr='mean', **kwargs):
        ...
        self.edge_mlp = mlp([all_node_dim + edge_dim, 16, 1])
    ...
    def edge_update(self, edge_index, x, node_feat, edge_attr):
        ...
        out = self.edge_mlp(tmp)

        return torch.cat([out, edge_attr[:, 1].unsqueeze(-1)], dim=1)
...
class ApSelectConv(MessagePassing):
    def __init__(self, node_dim: Dict, edge_dim, metadata: Metadata, aggr='mean', **kwargs):
        super(ApSelectConv, self).__init__(aggr=aggr)
        all_node_dim = sum(node_dim.values())
        self.ap_mlp = mlp([all_node_dim + edge_dim, 16])
        self.ap_mlp = Seq(*[self.ap_mlp, Seq(Lin(16, 1, bias=True), Sigmoid())])
    ...
    def forward(
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            x_j = x_dict[src_type]
            x_i = x_dict[dst_type]
            if edge_type == 'ue__uplink__ap':
                edge_dict[original_edge] = self.edge_updater(edge_index, x=x_j, node_feat=x_i, edge_attr=edge_attr)
        edge_dict['ap', 'downlink', 'ue'] = edge_dict['ue', 'uplink', 'ap']
...
class HetNetGNN_v4(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict):
        ...
        x_dict, edge_attr_dict = self.conv3(x_dict=x_dict, edge_index_dict=edge_index_dict,
                                            edge_attr_dict=edge_attr_dict)
        x_dict, edge_attr_dict = self.conv4(x_dict=x_dict, edge_index_dict=edge_index_dict,
                                            edge_attr_dict=edge_attr_dict)
        ue_feat = x_dict['ue']
        # print(ue_feat)
        power = self.power_mlp(ue_feat)
